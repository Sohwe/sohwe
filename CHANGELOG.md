...
- **Searchable GitHub repository picker.** The new-application dialog now filters repositories across every connected GitHub installation by repository or organization name, reports loading/API/empty states instead of silently hiding the picker, and still permits a Git URL to be entered manually.
...
- **One variable list with a per-variable scope (Railway-style).** The Variables page is now a single list instead of two editors: every variable carries **Build + runtime** (the default), **Runtime only**, or **Build only**, so a value needed in both places — `NEXT_PUBLIC_*`, `NODE_ENV` — is typed once rather than kept in sync by hand. An existing variable's scope can be changed from the list without revealing its value (`PATCH .../variables` with `rescope`), and narrowing a scope removes the value from the map it left, so moving a key off the build actually stops it reaching the image. New route `GET/PUT/PATCH /api/applications/:id/variables` (admin-and-above, masked by default, `?reveal=true` audited per map it exposed).

  **No storage change.** The scope is derived from which of the app's two existing encrypted maps hold the key — nothing new is stored, no migration runs, bundles keep format v3, and the worker is untouched. Existing apps keep exactly the behavior they had: today's env vars read as *Runtime only* and today's build variables as *Build only*, until someone changes them. The per-map routes (`/env`, `/build-args`) still work and remain how datastore bindings and bundle restore write one map without disturbing the other; a key the two maps disagree about is flagged in the list rather than silently resolved. The editor also warns when a key that looks like a credential is about to be exposed to the build, since build values land in image layers.
...
- **Custom domains get their own tab, and an app can have several.** Domains move out of the settings form onto a dedicated **Domains** tab per app. An app now carries a list of hostnames rather than one — apex plus `www`, a vanity domain, a cutover domain kept alive during a migration — and every one of them becomes a `Host()` term on the container's Traefik router. One domain is the **primary**: it is the app's headline URL in the dashboard and leads the router rule. Adding the first domain makes it primary automatically, promoting another is one click, and deleting the primary promotes the oldest survivor rather than leaving the app with no URL. Routing changes take effect on the app's next deploy, which the page says.

  What you paste is normalized before it is saved: `https://App.Example.com/pricing` is accepted and stored as `app.example.com`, with the normalized form shown before you submit. The same schema validates in the browser and on the server, so the inline error and the API's answer agree. Two claims are refused outright, because both would make Traefik cross-route traffic: a hostname another application on the instance already holds (409, and it does not say which app — that may be another organization), and a hostname that is some app's built-in `<slug>.<base-domain>` address.

  **Breaking (schema).** The `applications.domain` column is dropped in favour of a new `domains` table. The migration copies every configured domain across as that app's primary domain first, so nothing is lost; where two apps had named the same hostname — which the old column allowed but Traefik could only ever route to one of them — the older app keeps the claim and the shadowed duplicate is dropped. There is no down-migration. `PATCH /api/applications/:id` no longer accepts a `domain` field (a `domain` in the body is ignored rather than rejected); `POST /api/applications` still accepts one as a shortcut for the app's first domain. The API still returns `domain` on every application row, now derived from whichever domain is primary, so existing callers are unaffected.

- **www ↔ apex pairing with a real redirect (Vercel-style).** Adding an apex domain (`example.com`) on the Domains tab now offers — checked by default — to also add its `www.` variant and redirect it to the apex; typing `www.example.com` offers the reverse. The offer only appears at the zone apex (a curated list of compound TLDs like `.co.uk` and `.com.gh` counts): entering a real subdomain such as `app.example.com` shows nothing, because nobody expects `www.app.example.com` to exist. The redirect is served by Traefik itself as a permanent (308-class) redirect straight to the target's canonical scheme — one hop, `http://www.example.com/x` → `https://example.com/x` — and under HTTPS the redirecting hostname still gets its own certificate, since `https://www.example.com` must present a valid cert before it can redirect anywhere. Any domain can be flipped between serving and redirecting from its row (redirect to the primary in one click, or serve directly again); redirects cannot chain, cannot target a hostname the app does not hold, and a redirecting domain can never be the primary. Deleting a redirect's target clears the redirect on its dependents rather than bouncing visitors into a dead name. Like every routing change, redirects take effect on the app's next deploy. New nullable `domains.redirect_to` column (purely additive migration), new route `POST /api/applications/:id/domains/:domainId/redirect` (admin-and-above), `redirectTo` accepted on domain create, audited as `domain.redirect`. Config bundles do not yet carry redirect flags — a restored app serves both hostnames directly until the redirect is re-applied.

- **Per-domain DNS verification.** Every domain is checked the moment it is added and re-checkable from its row: the API walks the domain's NS records server-side to find the zone apex, maps the nameservers against a curated registry (Cloudflare — including Foundation DNS nameservers — Namecheap, GoDaddy, Route 53, Google Cloud DNS, DigitalOcean, Vercel, Porkbun, Linode, OVH, Gandi, Name.com, IONOS, Hetzner; unknown hosts fall back to showing the raw nameservers), and compares the domain's current A records against the instance's own IP, resolved from `SOHWE_BASE_DOMAIN` or a wildcard label under it. The result (`verified` / `points elsewhere` / `no record yet` / `can't verify`) is cached on the domain row, so the list renders without a lookup per row, and `verifiedAt` only moves forward — a transient resolver failure does not erase the fact that a domain was once seen pointing here. Expanding a row shows the exact record to create, with copy buttons, and a deep link into that provider's DNS console scoped to the detected zone. Reading and verifying are member-level; adding, removing, and re-pointing domains are admin-and-above. Routes: `GET/POST /api/applications/:id/domains`, `POST /api/applications/:id/domains/:domainId/verify`, `POST .../primary`, `DELETE .../:domainId`, plus the existing ad-hoc `GET /api/dns/inspect`. Audited as `domain.create`, `domain.delete`, and `domain.primary`.

- **One-click record apply, now for Cloudflare, DigitalOcean, and Hetzner.** An admin can store an org-level API token per provider; it is verified against that provider's API on save, encrypted at rest with `SOHWE_ENCRYPTION_KEY`, and never returned by any endpoint — the list route reports only that a credential exists. With a token configured, **Add record automatically** discovers the zone (longest dot-boundary match among the zones the token can see) and creates or updates the A record without leaving the dashboard. Which provider is used is decided by the domain's own nameservers, not by whichever credentials happen to be stored, so a token for the wrong provider cannot quietly write a record that changes nothing. Across all three drivers the token goes in a request header and nowhere else — never into a thrown message, which surfaces in API responses and logs — and a CNAME already on the name is refused, never overwritten. Cloudflare-created records are DNS-only (not proxied) so verification and ACME work out of the box, and updating keeps the record's existing proxied setting. Routes: `GET/PUT/DELETE /api/dns/credentials/:provider` (admin-and-above), `GET /api/dns/providers`, `POST /api/applications/:id/domains/:domainId/dns/apply` (admin-and-above, replacing the app-level `POST /api/applications/:id/dns/apply`). Audited as `dns.credentials.set`, `dns.credentials.delete`, and `dns.record.apply`, with no token material in audit rows, logs, or error messages. Adding a fourth provider is one `DnsDriver` implementation plus a registry entry; Namecheap is still deliberately out, since its `setHosts` call replaces a domain's entire record set in one shot.

- **Bundle format v4: every domain, not just one.** Config bundles now carry a per-app `domains` array alongside the existing `domain` (which stays, as the primary), so a restored app comes back with all of its hostnames. v1–v3 bundles still parse, with their single `domain` widened into a one-element list. Because hostnames are unique instance-wide, a restore drops any domain another app on the target instance already answers on and reports the count (`domainsSkipped`) rather than failing the whole restore.

- **`GET /api/config` reports `httpsEnabled`.** The dashboard needs it to say whether a custom domain will actually get a certificate, instead of offering an `https://` link that times out on an instance where `SOHWE_HTTPS_ENABLED` is off.
...
- **The DNS assist could tell you to point a domain at Cloudflare itself.** Sohwe works out the address a custom domain must point at by resolving `SOHWE_BASE_DOMAIN`. If that base domain is behind Cloudflare's orange cloud, resolving it returns a Cloudflare *edge* address rather than the server — so the Domains panel would show **Required record: A → 172.67.x.x**, "Add record automatically" would write exactly that, and Cloudflare would then fetch the origin from itself and serve **Error 1000: DNS points to prohibited IP**. The check then verified clean, because the domain really did resolve to the address Sohwe had asked for: a broken site with a green badge.

  Any address inside a known proxy network (Cloudflare's published IPv4 ranges) is now refused as an origin rather than returned. When that happens the panel says so and names the fix instead of suggesting a record, the auto-apply refuses with the same explanation, and nothing can report `verified`. A domain that itself resolves to a proxy edge gets a new **`proxied`** status — neither `verified` (wrong, and quiet) nor `mismatch` (alarming, and wrong), since the origin behind a proxy is unknowable from outside.

- **New `SOHWE_PUBLIC_IP`.** States this host's public IPv4 outright instead of inferring it, which is the supported answer for an instance whose base domain is legitimately proxied. Optional and validated at boot; the installer now records the address it already detects for its DNS guidance, so fresh installs get it for free. Threaded through `scripts/install.sh` and `docker-compose.prod.yml`.

- **A flaky build cache no longer fails a release that already published.** The release workflow's GitHub Actions cache export runs *after* the images are pushed, so an Actions Cache hiccup could mark a release failed when all three images and all their tags were already live on GHCR — which is what happened on `v0.6.4` (`error writing layer blob: not_found`). The cache export is now `ignore-error=true`, so it degrades to a cold cache instead of a red release; a genuine build or push failure still fails the job. The image matrix also drops `fail-fast`, so one leg failing can no longer cancel a sibling *mid-push* and leave one image's tags pointing at a new release while another's point at the old one.

- **Raw Prisma errors reaching the dashboard as toasts.** Creating an application with a slug that already exists surfaced Prisma’s own text — "Invalid prisma.application.create() invocation: Unique constraint failed on the fields: (organization_id, slug)" — database vocabulary, in a toast, with no hint that the fix is to change one field. The API now checks the slug up front and returns `409` with "An application with the slug “x” already exists in this organization", and the create dialog shows it on the slug field instead of in a toast, so the message is still there while it is being fixed. A new server-wide error handler (`apps/api/src/prisma-errors.ts`) is the backstop for every other route: unique violations become `409` naming only the meaningful column (scoping columns like `organization_id` are dropped), missing rows `404`, and any other Prisma failure a plain `500` "Database error." with the original logged server-side rather than described to the caller. Datastore creation, which had its own ad-hoc version of this check, now uses the shared helper and names the slug too.
