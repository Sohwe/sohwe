...
- **Phase 8 (custom domain DNS assist, first slice)** — the app settings page detects where a custom domain's DNS is hosted (Cloudflare, Namecheap, GoDaddy, Route 53, and more), shows live verification of the required record with copy-paste values and a deep link to the provider's DNS console, and — with an org-level Cloudflare API token, stored encrypted — creates or updates the record in one click. On `main`, unreleased.
