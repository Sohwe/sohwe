...
11. [Phase 8 — Custom Domains](#phase-8--custom-domains)
...
**Runtime version defaults.** Nixpacks resolves a Node version from `NIXPACKS_NODE_VERSION`, `engines.node`, `.nvmrc`, and `.node-version`, in that order, and otherwise uses its own `DEFAULT_NODE_VERSION` of 18. That last step is a trap: Node 18 is end-of-life, so an unpinned repo fails deep inside `npm run build` with an error that names the framework, not the runtime. `packages/builder/src/node-version.ts` substitutes Node 22 for that case alone — every explicit pin wins, `"*"` counts as no pin because Nixpacks treats it that way too, and Dockerfile builds are left entirely alone since their base image already decided. Deliberately no framework-to-runtime inference: mapping `next@16` to a required Node range would mean carrying a compatibility table that goes stale every release, to answer a question a current LTS already answers.

**Two variable maps, one scoped list.** The dashboard presents an app's variables the way Railway does — a single list where each entry is scoped `runtime`, `build`, or `both` — while storage stays two independent encrypted maps. The scope is *derived* from which maps hold a key, never stored, so the unified surface adds no state to migrate and the deploy path reads exactly the columns it always did. A key the two maps disagree about is reported to the user rather than silently resolved.

**Why the maps are separate underneath.** Runtime env vars are decrypted only for Docker `Env` injection, long after the image exists — nothing in a build can see them. Build variables are a second encrypted map on the application, passed to `nixpacks build --env KEY=value` or `docker build --build-arg KEY` (name-only, so values never land on an argv that `ps` can read). They are the only lever on the build itself, which is what makes `NIXPACKS_NODE_VERSION` and build-time-inlined values like `NEXT_PUBLIC_*` reachable. The split is not cosmetic: build variables are baked into image layers and readable via `docker history`, so conflating them with runtime secrets would quietly widen what a pulled image discloses. `packages/builder` owns the argv construction (`nixpacksArgv` / `dockerBuildArgv`, both pure and unit-tested) and scrubs every build variable value out of forwarded tool output.
...
3. The operator chooses a personal account or organization in GitHub's installation picker and selects repositories. GitHub returns it to `GET /api/github/setup/callback`, which verifies the supplied `installation_id` really belongs to this app (via `GET /app/installations/:id` with an app JWT) before storing an installation row. **Add account or organization** repeats this step; repository listing combines every connected installation.
...
**Webhook.** `POST /api/webhooks/github` is registered inside an encapsulated Fastify scope with a raw-buffer content-type parser, because `X-Hub-Signature-256` must be verified against the exact bytes GitHub signed — a re-serialized object will not match. Which app signed a delivery cannot be known before verification (the payload is untrusted), so the handler tries each connected app's secret. Only after the HMAC matches is the payload parsed, and installation-aware events must then match an installation an authenticated Sohwe admin connected. Pushes to a tracked branch enqueue a deploy for every matching `autoDeploy` app; tag pushes and branch deletions are ignored, and an installation deletion removes only that installation row.
...
*As built (Domain Connect deferred).*

Setting a custom domain used to be one field in the settings form that ended with "and DNS to this host" — the user was on their own to figure out where their DNS lives and what record to create, and an app could only ever have one. Domains now have their own tab, an app carries as many as it needs, and the page answers both questions itself.

**The list is the source of truth.** The `applications.domain` column is gone; a `Domain` row per hostname replaces it, with a `isPrimary` flag naming the app's headline URL. Hostnames are unique **instance-wide**, not per app: Traefik routes by host across the whole instance, so two apps claiming one hostname means conflicting router definitions and cross-routed traffic. The API still returns a `domain` field on every application row, derived from whichever domain is primary — a projection, not a second store. The worker turns the whole list into `Host()` terms on one router rule, primary first, deduplicated against the generated `<slug>.<base-domain>` address.

**Validation before storage.** `CreateDomainSchema` normalizes what people actually paste — a copied address bar, a port, a trailing root dot, mixed case — down to a bare hostname, then applies the same hostname check the old field used. The dashboard runs that schema as you type, so the inline error and the API's answer cannot disagree. Two claims are refused with an explanation rather than a constraint error: a hostname another app already holds (409, without naming that app — it may belong to another organization), and a hostname that is some app's built-in `<slug>.<base-domain>` address.

**Detection.** `apps/api/src/dns/inspect.ts` walks the domain's labels toward the root with server-side NS lookups until a zone answers — `app.example.com` → `example.com` — then maps the nameservers against a curated registry (`apps/api/src/dns/providers.ts`): Cloudflare (including its Foundation DNS enterprise nameservers), Namecheap, GoDaddy, Route 53, Google Cloud DNS, DigitalOcean, Vercel, Porkbun, Linode, OVH, Gandi, Name.com, IONOS, Hetzner. This identifies the **DNS host**, not the registrar — a Namecheap-registered domain on Cloudflare nameservers reports Cloudflare, which is where the record has to go. Unknown nameservers are shown raw rather than guessed at.

**Knowing this host's address.** Everything downstream depends on one number: the IPv4 a custom domain has to point at. `SOHWE_PUBLIC_IP` states it outright and wins when set — the only source that cannot be wrong about which machine this is. Otherwise it is inferred by resolving `SOHWE_BASE_DOMAIN`, falling back to a wildcard probe label since hosts commonly publish only `*.<base-domain>`. That inference has a trap: if the base domain is itself behind a proxy, resolving it returns an *edge* address, and advising someone to point their domain there makes the proxy fetch its origin from itself — Cloudflare's "Error 1000: DNS points to prohibited IP". It would also verify clean, since the domain then resolves to exactly the address Sohwe asked for. So an address inside a known proxy network (`apps/api/src/dns/cdn.ts`) is refused rather than returned, and the panel reports what is wrong and how to fix it instead of suggesting a record. Better no answer than a confidently wrong one.

**Verification.** The expected address is compared against the domain's current A records: `verified`; `proxied` when the domain resolves to a proxy edge, which is a working setup whose origin cannot be seen from outside and so is neither a pass nor a failure; `mismatch`; `unresolved`; or `unknown` when the instance's own IP could not be established. Adding a domain runs this check in the same request, so the panel opens on the record still needed; the result is cached on the row so the list renders without a lookup per row, and `verifiedAt` only moves forward — a transient resolver failure must not erase the fact that a domain was once seen pointing here. Both the ad-hoc `GET /api/dns/inspect` and the per-domain verify route are member-and-above; nothing they return is secret.

**Record apply.** Providers sit behind one contract (`apps/api/src/dns/driver.ts`): verify a token, find the zone, upsert an A record. Drivers exist for Cloudflare, DigitalOcean, and Hetzner — the providers whose API takes a single bearer token an operator can paste in. An admin stores one org-level token per provider, verified against that provider's API on save, encrypted with the instance key in the `DnsProviderCredential` table, and never returned by any endpoint; the list route reports existence only. `POST /api/applications/:id/domains/:domainId/dns/apply` (admin-and-above) picks the driver from the *domain's own nameservers* rather than from whichever credentials happen to be stored — writing a record at a provider the zone does not live on would change nothing — finds the zone by longest dot-boundary suffix match, then creates or updates the A record. Two rules hold across every driver: the token goes in a request header and nowhere else, never into a thrown message that surfaces in a response or a log; and an existing CNAME on the name is a refusal, not an overwrite. Cloudflare-created records are DNS-only (`proxied: false`) so verification and ACME work immediately, and updates keep the record's existing proxied flag — switching off someone's deliberate orange cloud is not this feature's call. Everything is audited (`domain.*`, `dns.credentials.*`, `dns.record.apply`) with no token material anywhere.

**Deliberately not done:** Namecheap's API (its `setHosts` replaces the domain's entire record set in one call — one bug away from deleting someone's MX records), and the Domain Connect flow, which needs a Sohwe service template registered with each participating provider and so is a separate effort. Every DNS and provider module takes injected resolver/fetch dependencies, so the whole surface is unit- and route-tested without touching the network.
