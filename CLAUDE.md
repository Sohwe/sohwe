...
- `apps/api/.env` - `DATABASE_URL`, `REDIS_URL`, `PORT`, `SESSION_SECRET`, `SOHWE_ENCRYPTION_KEY`, `SOHWE_BASE_DOMAIN`, `SOHWE_PUBLIC_IP` (optional), `SOHWE_HTTPS_ENABLED`, `SOHWE_CERT_RESOLVER`
...
- The address a custom domain must point at comes from `resolveExpectedIp` (`apps/api/src/dns/inspect.ts`): `SOHWE_PUBLIC_IP` first, else resolved from `SOHWE_BASE_DOMAIN`. An address inside a known proxy network (`cdnForAddress` in `apps/api/src/dns/cdn.ts`) is **refused, never returned** — a proxied base domain resolves to an edge address, and suggesting it makes the proxy fetch its origin from itself (Cloudflare Error 1000) while verifying clean. Anywhere an address is treated as an origin, run it past `cdnForAddress` first, and keep the `proxied` status distinct from both `verified` and `mismatch`.
...
- `GET /api/config` is public and allowed through the setup gate (`apps/api/src/setup-gate.ts`); it exists so the dashboard learns `SOHWE_BASE_DOMAIN` and the running release tag at runtime instead of baking either into the SPA. The release workflow passes the exact Git tag as the Docker `SOHWE_VERSION` build arg; do not source the sidebar value from the installer's host-side `SOHWE_VERSION`, because that may intentionally remain the mutable `latest` alias.
